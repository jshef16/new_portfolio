...
function showSlides(n) {
  ...
  // Animate the percent-bars in the currently active slide
  let tl_skills = gsap.timeline({
    defaults: { ease: "bounce", duration: 2 }
  });

  // Only target the .percent-bar elements in the visible (active) slide
  let percentBars = slides[slideIndex - 1].getElementsByClassName('percent-bar');

  tl_skills.from(percentBars, { width: 0, stagger: 0.25 });
}
